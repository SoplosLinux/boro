#!/bin/bash
# Script de instalación para repositorio boro
...
if ! wget -qO - https://raw.githubusercontent.com/SoplosLinux/boro/main/public.key | gpg --dearmor -o /usr/share/keyrings/boro.gpg; then
    echo -e "${RED}Error al descargar la clave GPG${NC}"
    exit 1
fi

# Verificar que la clave se descargó correctamente
if [ ! -s /usr/share/keyrings/boro.gpg ]; then
...
echo "deb [signed-by=/usr/share/keyrings/boro.gpg trusted=yes] https://github.com/SoplosLinux/boro/raw/refs/heads/main/ stable main" > /etc/apt/sources.list.d/boro.list

# Forzar actualización segura
echo -e "${GREEN}Actualizando índices...${NC}"
rm -f /var/lib/apt/lists/*boro*
apt-get update --allow-insecure-repositories

# Verificar que el repositorio está configurado
if ! apt-cache policy | grep -q "https://github.com/SoplosLinux/boro/raw/refs/heads/main"; then
